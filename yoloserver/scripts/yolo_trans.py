    ...
    def _process_single_split(self, label_files: List[Path], image_paths: List[Path], split_name: str):
        ...
        for image_path in image_paths:
            ...
            try:
                ...
                logger.debug(f"复制图像文件 {image_path.name} 到 '{new_image_path.relative_to(YOLOSERVER_ROOT)}' 成功")
        ...
        for label_file_path in label_files:
            ...
            try:
                ...
                logger.debug(f"复制标签文件 {label_file_path.name} 到 '{new_label_file.relative_to(YOLOSERVER_ROOT)}' 成功")
            except Exception as e:
