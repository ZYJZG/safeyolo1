...
from configs import COMMENTED_TRAIN_CONFIG, COMMENTED_VAL_CONFIG, COMMENTED_INFER_CONFIG, DEFAULT_INFER_CONFIG, DEFAULT_TRAIN_CONFIG, DEFAULT_VAL_CONFIG,RUNS_DIR
import argparse
...
def generate_default_config(config_type: str='train'):
    ...
    if config_type == 'train':
        ...
        raise ValueError(f"未知的配置文件类型:{config_type}, 当前仅支持[train,val,infer]三种模式")
    try:
        with open(config_path, 'w', encoding='utf-8') as f:
            f.write(config)
            logger.info(f"生成默认{config_type}配置文件成功:{config_path}")
    except IOError as e:
        logger.error(f"生成默认{config_type}配置文件失败:{e}")
        raise e
    except Exception as e:
        logger.error(f"生成默认{config_type}配置文件发生未知错误:{e}")
        raise e
...
def merger_configs(args,yaml_config,mode='train'):
    ...
    if hasattr(args,"use_yaml") and args.use_yaml and yaml_config:
        ...
        logger.debug(f"已经合并YAML参数:{merged_params}")
    ...
    if hasattr(args,"extra_args"):
        if len(args.extra_args) %2 != 0:
            logger.error(f"额外参数个数错误,必须成对出现:如(--key value)")
...
def log_parameters(args, exclude_params=None):
    '''
    记录命令行与yaml参数信息，返回结构化字典
    :param args:
    :param exclude_params:
    :return:
    '''
    if exclude_params is None:
        exclude_params = ['ues_yaml', 'log_level', 'extra_args']
        ...
        for key, value in vars(args).items():
            if key not in exclude_params and not key.endswith('_specified'):
                ...
                logger.info(f"{key:<30}{value} 来源{source:<20}")
